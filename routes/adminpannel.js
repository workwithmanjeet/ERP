...
router.post('/adminpannel/new', isLoggedIn,isAdmin,catchAsync( async (req,res)=>{
    // console.log(req.body);
    // res.send(req.body)
    var filter = req.body.filter
    const d=new Date()
    filter.age= d.getFullYear()-Number(filter.dob.slice(0,4))
    const cc =await Counter.findById('61e04c7b4d9d6c89fa00f24e')
    // console.log(cc)
...
router.get('/adminpannel/s/:id', isLoggedIn, isAdmin,catchAsync(async (req,res)=>{
    const {id}=req.params
    console.log(id)
...
router.delete('/adminpannel/s/:id', isLoggedIn,isAdmin,catchAsync(async (req,res) =>{
    const {id} = req.params;
...
router.patch('/adminpannel/s/:id', isLoggedIn,isAdmin,catchAsync(async (req,res) =>{
    const {id} = req.params;
    // console.log(req.body);
...
router.post('/adminpannel/views', isLoggedIn,isAdmin,catchAsync(async (req,res)=>{
    let fil={}
